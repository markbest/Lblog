				   ...
				   @foreach ($pictures as $picture)
					  <li class="picture_wall_list">
						  <form name="pic_info" action="{{ URL('admin/picture/'.$picture->id)}}" method="post">
							  ...
							  <a rel="group" href="{{ storage_path('app/public/image') . DIRECTORY_SEPARATOR . $picture->img_url }}" title="{{ $picture->note }}">
								  <img src="{{ storage_path('app/public/image') . DIRECTORY_SEPARATOR . $picture->img_url }}" />
							  </a>
